...
class TestImageController(unittest.TestCase):
    ...
    def test_adjust_brightness_contrast(self):
        ...
        # 使用极端值进行亮度增加测试（增加100）
        self.controller.adjust_brightness_contrast(100, 1.0)
        bright_image = self.model.current_image.copy()
        
        # 验证至少有些像素变亮了
        self.assertTrue(np.mean(bright_image) > np.mean(original_image), 
                      f"平均亮度应该增加，原始：{np.mean(original_image)}，调整后：{np.mean(bright_image)}")
        ...
        # 验证图像有变化
        self.assertTrue(np.any(contrast_image != original_image),
                      "对比度调整后图像应该有变化")
